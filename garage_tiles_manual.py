...
st.title("Garage Tile Designer v3.11 - Clickable Grid")
...
ancho_input = st.number_input(f"Ancho ({unidad})", min_value=min_val, 
                              value=4.0 if unidad=="metros" else 400.0, step=1.0)
largo_input = st.number_input(f"Largo ({unidad})", min_value=min_val, 
...
pos_b = st.multiselect("¿Dónde colocar bordillos?", ["Arriba","Abajo","Izquierda","Derecha"], 
...
# Capturar clics
sel = plotly_events(fig, click_event=True, key="plotly_click")
if sel:
    c = int(sel[0]['x'])
    r = int(sel[0]['y'])
